...
interface ReceivablesProjectionProps {
  contracts: Contract[];
...
export function ReceivablesProjection({ contracts, onContractClick }: ReceivablesProjectionProps) {
  ...
  const filteredContracts = useMemo(() => {
    ...
    // Filtrar apenas contratos com valor a receber
    let filtered = contracts.filter(c => c.amount_to_receive > 0);

    if (period === 'all') {
      return filtered.sort((a, b) => b.amount_to_receive - a.amount_to_receive);
    }
  ...
  const totalToReceive = useMemo(() => {
    return filteredContracts.reduce((sum, c) => sum + c.amount_to_receive, 0);
  }, [filteredContracts]);
      ...
      <CardContent className="space-y-4">
            ...
            filteredContracts.map((contract) => (
              ...
              >
                ...
                <div className="text-right ml-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    de {formatCurrency(contract.contract_value)}
                  </p>
                </div>
              </div>
            ))
        ...
        <div className="pt-2 border-t">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-green-600">{formatCurrency(totalToReceive)}</span>
          </div>
        </div>
      </CardContent>
    </Card>
